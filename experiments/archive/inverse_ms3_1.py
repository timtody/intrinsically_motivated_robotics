...
class Experiment(BaseExperiment):
    ...
    def plot(results):
        ...
        def process_data(data):
            data_pre = pd.DataFrame({"x": x.ravel(), "y": y.ravel(), "norm": data})
            data_pre = data_pre.pivot("x", "y", "norm")
            return data_pre
        ...
        cb.set_label(label=r"$||a^i_{x,y}||_2$", weight="bold", size=16)

        plt.savefig("test_norm.pdf", bbox_inches="tight")
        plt.show()
