...
class Experiment(BaseExperiment):
    ...
    def compute_matrix(self):
        ...
        results = np.zeros((Experiment.grid_size ** 2, 2))
    ...
    def plot(results):
        ...
        def process_data(data):
            data_pre = pd.DataFrame(
                {"x": x.ravel(), "y": y.ravel(), "u": data[:, 0], "v": data[:, 1]}
            )
        ...
        for i in range(n_procs):
            ...
            data_pre.to_csv(f"results/ms3/arrow_rank{i}_pre.csv")
            data_post.to_csv(f"results/ms3/arrow_rank{i}_post.csv")
